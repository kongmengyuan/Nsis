...
!define MUI_PAGE_LICENSE_RTY "0_license_多用户发布专用标题\license.rtf" ; 发布成为不同的文件名

;!define PRODUCT_NAME "HudunIPhoneRecovery" ; 发布成为不同的文件名(安装目录名,一般用统一的英文目录)
!define PRODUCT_NAME "多用户发布专用标题" ; 发布成为不同的文件名(安装目录名,一般用统一的英文目录)
...
Function .onInit
    ...
    File `/ONAME=$PLUGINSDIR\banner1.bmp` `0_images_多用户发布专用标题\banner1.bmp` ; 发布成为不同的文件名

    File `/ONAME=$PLUGINSDIR\btn_agree.bmp` `images\btn_agree.bmp`
    ;File `/oname=$PLUGINSDIR\license.rtf` `license\license.rtf` ; 发布成为不同的文件名
    File `/oname=$PLUGINSDIR\license.rtf` `0_license_多用户发布专用标题\license.rtf` ; 发布成为不同的文件名
    ...
    File `/ONAME=$PLUGINSDIR\banner2.bmp` `0_images_多用户发布专用标题\banner2.bmp` ; 发布成为不同的文件名
    ...
    File `/ONAME=$PLUGINSDIR\banner3.bmp` `0_images_多用户发布专用标题\banner3.bmp` ; 发布成为不同的文件名
...
Function InstallationMainFun
  ...
  File "0_Files\多用户发布专用标题\WchatRecovery.exe" ; 发布成为不同的文件名
  File "Files\set.ini"
  File "Files\*.dll"
...
Function CreateLink
    ...
    CreateShortCut "$DESKTOP\多用户发布专用标题.lnk" "$INSTDIR\WchatRecovery.exe" "" "$INSTDIR\WchatRecovery.exe" ; 发布成为不同的文件名,最后面的WchatRecovery.exe是图标文件,直接写图标文件名有时不好用,很奇怪
    CreateDirectory "$SMPROGRAMS\多用户发布专用标题" ; 发布成为不同的文件名
    CreateShortCut "$SMPROGRAMS\多用户发布专用标题\卸载 多用户发布专用标题.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe" ; 发布成为不同的文件名 ,uninstall.ico不好用就使用uninstall.exe
    CreateShortCut "$SMPROGRAMS\多用户发布专用标题\多用户发布专用标题.lnk" "$INSTDIR\WchatRecovery.exe" "" "$INSTDIR\WchatRecovery.exe" ; 发布成为不同的文件名
...
Function WriteReg
  ...
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\多用户发布专用标题" "DisplayIcon" '"$INSTDIR\WchatRecovery.exe"'  ; 发布成为不同的文件名
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\多用户发布专用标题" "DisplayVersion" "1.0.0.0"  ; 发布成为不同的文件名
  ...
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\多用户发布专用标题" "HelpLink" "http://Wx.tianduntech.com"  ; 发布成为不同的文件名
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\多用户发布专用标题" "InstallLocation" "$INSTDIR" ; 发布成为不同的文件名
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\多用户发布专用标题" "URLInfoAbout" "http://Wx.tianduntech.com"  ; 发布成为不同的文件名
...
Function CompletePage
    ...
    ExecShell "open" "http://url.huifudashi.com/1/buy/welcome/"  ; 软件安装完成后，弹出链接
...
Function un.onInit
  ...
  File `/ONAME=$PLUGINSDIR\banner4.bmp` `0_images_多用户发布专用标题\banner4.bmp` ; 发布成为不同的文件名
...
Function un.onClickUninstall
  ...
  ExecShell "open" "http://www.huifudashi.com/buy_discount.html" ; 软件卸载时，弹出链接
