...
SetCompress force
XPStyle on
;RequestExecutionLevel admin ;获取管理员权限,和下面的代码“SetShellVarContext all”是成对使用.  none|user|highest|admin , none和admin的权限差不多,可用F1里面查到相关解释
RequestExecutionLevel user ;只要求普通用户权限级别(不需要管理员组权限),反正目前是安装在AppData当前用户,所以不用管理员权限也可以,下面的代码“SetShellVarContext all”也要改为“SetShellVarContext Current”。 none|user|highest|admin , none和admin的权限差不多,可用F1里面查到相关解释
...
;Page custom PageMutiLanguage ; 多语言选择,需要 !include "MUI2.nsh" ,这个界面是好用的,只是目前全部的客户都是中国的,所以先不放.以后可以按国家发布,或者多语言的安装界面.
; 欢迎页面
Page custom PageWelcome
; 安装过程页面
Page custom PageLoading
; 安装完成页面
Page custom PageComplete
...
InstallDir "$APPDATA\多用户发布专用标题" ; 发布成为不同的文件名(安装目录名),Win8和Win10的特殊处理,现在就Win7以后都装在C:\ProgramData了
...
Function .onInit
    ...
    File `/ONAME=$PLUGINSDIR\language_zh_cn.bmp` `images\language_zh_cn.bmp`
    File `/ONAME=$PLUGINSDIR\language_en.bmp` `images\language_en.bmp`
    File `/ONAME=$PLUGINSDIR\language_ja.bmp` `images\language_ja.bmp`
    File `/ONAME=$PLUGINSDIR\language_zh_tw.bmp` `images\language_zh_tw.bmp`
    ...
    SkinBtn::Init "$PLUGINSDIR\checkbox1.bmp"
		...
		; 多语言选择
    SkinBtn::Init "$PLUGINSDIR\btn_next_exe.bmp"
    SkinBtn::Init "$PLUGINSDIR\btn_close_exe.bmp"
FunctionEnd
...
;取消所有焦点
Function CancleRadioButtionSelect
  ${NSD_Uncheck} $RadioButton1 ;取消焦点
  ${NSD_Uncheck} $RadioButton2 ;取消焦点
  ${NSD_Uncheck} $RadioButton3 ;取消焦点
  ${NSD_Uncheck} $RadioButton4 ;取消焦点
FunctionEnd
Function onClickPicSimplifiedChinese
	Call CancleRadioButtionSelect ;取消所有焦点
  ${NSD_Check} $RadioButton1 ;设置焦点 ;默认选择的是"中文简体"
FunctionEnd
Function onClickPicEnglish
	Call CancleRadioButtionSelect ;取消所有焦点
  ${NSD_Check} $RadioButton2 ;设置焦点 ;默认选择的是"English"
FunctionEnd
Function onClickPicJapanese
	Call CancleRadioButtionSelect ;取消所有焦点
  ${NSD_Check} $RadioButton3 ;设置焦点 ;默认选择的是"日本語"
FunctionEnd
Function onClickPicTraditionalChinese
	Call CancleRadioButtionSelect ;取消所有焦点
  ${NSD_Check} $RadioButton4 ;设置焦点 ;默认选择的是"中文繁體"
FunctionEnd
...
Function InstallationMainFun4.5
    ...
    File "Files4.5\PDF Convert.exe.config"
    File "Files4.5\PDF Convert.vshost.exe.config"

    SetOutPath "$INSTDIR\ar"
    SetOverwrite on ; 覆盖安装,比较安全,只对目录有效
    ; 修改时要注意不要修改文件的顺序,后面的85是时间,是根据实际情况测试过的,进度条是依据它来处理的
    SendMessage $PB_ProgressBar ${PBM_SETPOS} 88 0
    File "Files4.5\ar\*.*"
    SetOutPath "$INSTDIR\en"
    SetOverwrite on
...
Function PageMutiLanguage
  ...
  ${NSW_SetWindowSize} $HWNDPARENT 428 290 ; 改变窗体大小554,384/554,534
  ${NSW_SetWindowSize} $0 428 290 ; 改变Page大小

  ; 多语言选择贴图片,顶部图片(背景图)
;  ${NSD_CreateBitmap} 0,0,100%,100% "" ; X坐标,Y坐标,宽,高,文字
;  Pop $LanguageBgImage
;  ${NSD_SetImage} $LanguageBgImage $PLUGINSDIR\language_top.bmp $ImageHandle ; 加载图片,图片要在“Function .onInit”里面初始化
...
;  ${NSD_CreateGroupBox} 42.01u 26u 186.32u 118u "GroupBox1"
;  Pop $hCtl_GroupBox1

  ;${NSD_CreateRadioButton} 10 12u 100% 10u "中文简体"
  ${NSD_CreateRadioButton} 105u 53u 10u 10u ""
  Pop $RadioButton1
  ;SetCtlColors $RadioButton1 ""  transparent ; 背景设成透明
  ;${NSD_CreateRadioButton} 10 32u 100% 10u English
	${NSD_CreateRadioButton} 105u 75u 10u 10u ""
  Pop $RadioButton2
  ;SetCtlColors $RadioButton2 ""  transparent ; 背景设成透明
  ;${NSD_CreateRadioButton} 10 52u 100% 10u '日本語'
  ${NSD_CreateRadioButton} 105u 99u 10u 10u ""
  Pop $RadioButton3
  ;SetCtlColors $RadioButton3 ""  transparent ; 背景设成透明
  ;${NSD_CreateRadioButton} 10 72u 100% 10u '中文繁體'
  ${NSD_CreateRadioButton} 105u 121u 10u 10u ""
	Pop $RadioButton4

	;SetCtlColors $RadioButton1 ""  transparent ; 背景设成透明
  ${NSD_CB_SelectString} $RadioButton1 "中文简体" ;默认选择的是"中文简体"
  ${NSD_Check} $RadioButton1 ;设置焦点

  ;Next 下一页按钮(选择语言界面)
  ${NSD_CreateButton} 22 220 180 50 ""
  Pop $Btn_Next_Exe
  ...
  ;Close 关闭按钮(选择语言界面)
  ${NSD_CreateButton} 222 220 180 50 ""
  Pop $Btn_Close_Exe
  ...
  SkinBtn::onClick $1 $3
	...
	;贴图片要在其它控件加载完成之后再加载，如果写在顶部的话会看不见,KongMengyuan,2016-03-03
  ; 多语言选择贴图片,顶部图片
  ${NSD_CreateBitmap} 0 0 100% 100% "" ; X坐标,Y坐标,宽,高,文字
  Pop $LanguageBgImage
  ${NSD_SetImage} $LanguageBgImage $PLUGINSDIR\language_top.bmp $LanguageBgImageHandle ; 加载图片,图片要在“Function .onInit”里面初始化
  ; 处理无边框窗体移动
  GetFunctionAddress $0 onGUICallback
  WndProc::onCallback $LanguageBgImage $0 ; 处理无边框窗体移动

  ; 多语言选择贴图片,中文简体
  ${NSD_CreateBitmap} 43% 78 100% 100% "" ; X坐标,Y坐标,宽,高,文字
  Pop $LanguageBgImage
  ${NSD_SetImage} $LanguageBgImage $PLUGINSDIR\language_zh_cn.bmp $LanguageBgImageHandle ; 加载图片,图片要在“Function .onInit”里面初始化
  ; 处理无边框窗体移动
  GetFunctionAddress $0 onGUICallback
  WndProc::onCallback $LanguageBgImage $0 ; 处理无边框窗体移动
  ;点击事件,不增加此Click事件也可以使用,只是用户需要点击前面的"点"才可选择,为了更好用户体验,在图片上面也可以点击选中
  StrCpy $1 $LanguageBgImage
  GetFunctionAddress $3 onClickPicSimplifiedChinese ; 调用图片点击事件
  SkinBtn::onClick $1 $3

  ; 多语言选择贴图片,English
  ${NSD_CreateBitmap} 43% 112 100% 100% "" ; X坐标,Y坐标,宽,高,文字
  Pop $LanguageBgImage
  ${NSD_SetImage} $LanguageBgImage $PLUGINSDIR\language_en.bmp $LanguageBgImageHandle ; 加载图片,图片要在“Function .onInit”里面初始化
  ; 处理无边框窗体移动
  GetFunctionAddress $0 onGUICallback
  WndProc::onCallback $LanguageBgImage $0 ; 处理无边框窗体移动
  ;点击事件,不增加此Click事件也可以使用,只是用户需要点击前面的"点"才可选择,为了更好用户体验,在图片上面也可以点击选中
  StrCpy $1 $LanguageBgImage
  GetFunctionAddress $3 onClickPicEnglish ; 调用图片点击事件
  SkinBtn::onClick $1 $3

  ; 多语言选择贴图片,日本語
  ${NSD_CreateBitmap} 43% 148 100% 100% "" ; X坐标,Y坐标,宽,高,文字
  Pop $LanguageBgImage
  ${NSD_SetImage} $LanguageBgImage $PLUGINSDIR\language_ja.bmp $LanguageBgImageHandle ; 加载图片,图片要在“Function .onInit”里面初始化
  ; 处理无边框窗体移动
  GetFunctionAddress $0 onGUICallback
  WndProc::onCallback $LanguageBgImage $0 ; 处理无边框窗体移动
  ;点击事件,不增加此Click事件也可以使用,只是用户需要点击前面的"点"才可选择,为了更好用户体验,在图片上面也可以点击选中
  StrCpy $1 $LanguageBgImage
  GetFunctionAddress $3 onClickPicJapanese ; 调用图片点击事件
  SkinBtn::onClick $1 $3

  ; 多语言选择贴图片,中文繁體
  ${NSD_CreateBitmap} 43% 181 100% 100% "" ; X坐标,Y坐标,宽,高,文字
  Pop $LanguageBgImage
  ${NSD_SetImage} $LanguageBgImage $PLUGINSDIR\language_zh_tw.bmp $LanguageBgImageHandle ; 加载图片,图片要在“Function .onInit”里面初始化
  ; 处理无边框窗体移动
  GetFunctionAddress $0 onGUICallback
  WndProc::onCallback $LanguageBgImage $0 ; 处理无边框窗体移动
  ;点击事件,不增加此Click事件也可以使用,只是用户需要点击前面的"点"才可选择,为了更好用户体验,在图片上面也可以点击选中
  StrCpy $1 $LanguageBgImage
  GetFunctionAddress $3 onClickPicTraditionalChinese ; 调用图片点击事件
  SkinBtn::onClick $1 $3

  ; 贴背景大图,最后贴一个全窗体的白色背景作为底图,然后把这个图片处理成窗体移动,这样就避免了对每个图片设置窗体移动了
  ...
  nsDialogs::Show
	${NSD_FreeImage} $LanguageBgImageHandle ; 释放该图片内存区
